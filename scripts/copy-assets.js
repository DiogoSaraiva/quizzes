...
import { existsSync, copyFileSync, readdirSync, rmSync, readFileSync, writeFileSync } from "fs";
...
function detectParentBrand() {
	...
	if (existsSync(parentIndex)) {
		...
		if (match) {
			return {
				brand: match[1].trim(),
				brandMark: (match[2] || "").trim(),
			};
		}
	}

	return { brand: "Quizzes", brandMark: "" };
}

const detected = detectParentBrand();
const brandSource =
	"// Auto-generated by scripts/copy-assets.js\n" +
	`export const BRAND = '${escJs(detected.brand)}';\n` +
	`export const BRAND_MARK = '${escJs(detected.brandMark)}';\n`;
